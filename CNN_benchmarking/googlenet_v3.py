...
#vgg16.features[0] = torch.nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)
#googlenet.conv1 = torch.nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)
...
for epoch in range(num_epochs):
    ...
    running_loss = 0.0
    for inputs, labels in train_loader:  # Make sure you've defined train_loader
    ...
    total = 0
    with torch.no_grad():
        for inputs, labels in val_loader:  # Make sure you've defined val_loader
            inputs, labels = inputs.to(device), labels.to(device)
            outputs = googlenet(inputs)
...
torch.save(googlenet.state_dict(), './googlenet_trained.pth')

# Assuming the necessary imports are done and the device is set

# Load pre-trained googlenet model structure
googlenet = models.googlenet(pretrained=True)  # Set pretrained to False for loading custom weights later
...
#googlenet.load_state_dict(torch.load('./googlenet_trained.pth'))
#googlenet.conv1 = torch.nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)
state_dict = torch.load('./googlenet_trained.pth')
googlenet.load_state_dict(state_dict)

googlenet.eval()  # Start with eval mode
...
googlenet.fc = nn.Linear(googlenet.fc.in_features, num_new_classes)
...
torch.save(googlenet.state_dict(), './fine_tuned_googlenet.pth')
...
#googlenet.load_state_dict(torch.load('./fine_tuned_googlenet.pth'))
#googlenet.conv1 = torch.nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)
state_dict = torch.load('./fine_tuned_googlenet.pth')
googlenet.load_state_dict(state_dict)

googlenet.eval()  # Set the model to evaluation mode

# Model parameters and size
total_params, trainable_params = count_parameters(googlenet)
model_size = estimate_model_size_in_MB(googlenet)
...
def predict_image(image_path):
    ...
    with torch.no_grad():  # No need to compute gradients
        outputs = googlenet(image_tensor)
